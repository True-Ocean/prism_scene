...
def load_all_images_from_gcs(bucket_name, file_names_list):
    ...
    for name in file_names_list:
        blob = bucket.blob(f"{sub_dir_name}/{name}")
...
# Google Cloud Storage のバケット名
sub_dir_name = "archive/20251130_ジャパンカップ" # ================================= ここを書き換え =====================================
...
# --- メイン画面の表示 ---
st.title('🐎 2025年11月30日 ジャパンカップ 分析レポート') # ここを変更 =================================================================
st.divider()
...
if sub_menu == "基礎能力と先行指数":
    ...
    st.write(f"【横軸】 EPI:Early Position Index（先行指数）は、過去のレース実績のAve-3F（前半600m換算タイム）をもとに算出された指標で、数値が低いほどレース序盤での位置取りが有利であることを示します。")
    st.write(f"【縦軸】 基礎能力（偏差値）は、JRAで開催された過去20年間の同種レース実績データから得られる標準スコアをベースとした、各馬の偏差値を示します。")
    ...
    st.divider()
    st.subheader(f"💡 グラフ解説")
    st.write(f"2つのグラフは、PRISM_SCENE分析の中の、PRISM_G分析による結果を表しています。")
    ...
    st.divider()
    st.subheader(f"💡 グラフ解説")
    st.write(f"このグラフは、PRISM_SCENE分析の中の、PRISM_B分析による結果を表しています。")
    st.write(f"【横軸】 前走、前々走での調教における最終ラップタイムからの変化を示します。数値が高いほど、末脚の鋭さが上がっていることを示します。")
    st.write(f"【縦軸】 前走、前々走での調教における最終加速ラップからの変化を示します。数値が高いほど、加速の持続性が高くなっていることを示します。")
    ...
    st.divider()
    st.subheader(f"💡 グラフ解説")
    st.write(f"このグラフは、PRISM_SCENE分析の最終結果を表しています。")
    ...
    if Final_Mark_df is not None:
        ...
        st.subheader(f"💡 印の意味")
        st.write('')
        st.write('全世界シミュレーションの最終結果をベースに、以下の基準で注目キャラを選出しています。')
        st.markdown("""
        【 本  名 】: 平均着順が最も優秀なキャラ  
        【 対  抗 】: 平均着順が2番手のキャラ  
