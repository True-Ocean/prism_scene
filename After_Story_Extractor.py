...
def Actual_Race_Projector(cast_df, ensemble_df, final_result, client, model):
    ...
    system_instruction = f"""
        ...
        実況形式やスペック紹介は一切不要です。18頭の群像劇でありながら、特定の因縁に焦点を当てた、緩急のある物語を創出してください。
        """
...
def After_Story_Extractor(actual_drama, final_result, cast_df, ensemble_df, client, model):
    ...
    selected_scene = ""
    scenes = ['トレセンの中庭', 'トレセンの練習場', '寮の食堂', 'シャワールーム', '保健室', 'トレーニングルーム', 'カフェ', 'ショッピングモール', '遊歩道', '牧場']
    ...
    user_prompt = f"""
        ...
        - 誰にとっても気軽に読めるラノベ風の描写とし、馬同士の会話を中心としたストーリーを描いてください。

        【ストーリー展開】
        - ストーリーの冒頭では、{selected_time}の{selected_scene}の雰囲気について簡潔に描写してください
        - それぞれの馬は、{horses_order}の順に、そのシーンに現れるようしてください。その際、同時に登場する場合や、間隔をあけて登場する場合等、タイミングはランダムに設定してください。
        （例）2着馬と3着馬が会話している途中で、1着馬が現れる、等
        - ストーリーでは、レースの反省や次回レースへの思い、お互いの複雑な心情、相手に対する尊敬や熱いライバル心、などを描いててください。
        - レース当日とは違い、普段のプライベートな会話となるよう意識し、レースとは違った、よりリラックスした会話にしてください。
        - キャラ設定やライバル関係によって、真面目な会話だけでなく、冗談、皮肉、からかい、といった内容も散りばめてください。
        - 全体を1000文字程度に収めてください。

        【登場キャラ紹介】
        - ストーリーに続き、登場した3人のキャラの紹介（馬名：キャラ設定）を、1着、2着、3着の順に、簡潔に記載してください。
